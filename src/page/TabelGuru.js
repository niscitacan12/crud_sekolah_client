...
import { FaPenSquare, FaInfo, FaTrashAlt, FaPlus, FaSearch, FaAngleLeft, FaAngleRight } from 'react-icons/fa';
...
function TabelGuru() {
  ...
  const deleteGuru = async (id) => {
    ...
    await Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://localhost:7000/api/data_guru/hapus/${id}`, {
            headers: {
 ...
 const currentItems = guru.filter((dataGuru) =>
   dataGuru.nama_guru.toLowerCase().includes(searchTerm.toLowerCase())
 ).slice(indexOfFirstItem, indexOfLastItem);

 const paginate = (pageNumber) => {
   if (pageNumber < 1 || pageNumber > Math.ceil(guru.length / itemsPerPage)) return;
        ...
        <div className="bg-white shadow-md rounded-lg p-4">
              ...
              <tbody>
                {currentItems
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{dataGuru.nama_guru}</td>
                    <td className="px-6 py-4">{dataGuru.email}</td>
                    <td className="px-6 py-4">{dataGuru.jabatan}</td>
                    <td className="whitespace-nowrap text-center py-2">
                      <div className="flex items-center -space-x-4 hover:space-x-1">
                        <Link to={`/tabelGuru/ubahGuru/${dataGuru.id}`}>
                        <button className="z-20 block rounded-full border-2 border-white bg-blue-100 p-4 text-blue-700 transition-all hover:scale-110 focus:outline-none focus:ring active:bg-blue-50"
                        >
                            <FaPenSquare className="z-20" title="Edit" />
                        </button>
                        </Link>
                        <Link to={`/tabelGuru/detailGuru/${dataGuru.id}`}>
                          <button className="z-20 block rounded-full border-2 border-white bg-blue-100 p-4 text-blue-700 transition-all hover:scale-110 focus:outline-none focus:ring active:bg-blue-50">
                        ...
                        <button className="z-30 block rounded-full border-2 border-white bg-red-100 p-4 text-red-700 transition-all hover:scale-110 focus:outline-none focus:ring active:bg-red-50"
                          onClick={() => deleteGuru(dataGuru.id)}
                        >
          ...
          <div className="flex justify-between items-center mt-4">
            <div>
              ...
              <button
                onClick={() => paginate(currentPage + 1)}
                disabled={currentPage === Math.ceil(guru.length / itemsPerPage)}
                className={`px-3 py-1 rounded-md bg-blue-500 text-white ${
                  currentPage === Math.ceil(guru.length / itemsPerPage) ? "opacity-50 cursor-not-allowed" : ""
                }`}
              >
            ...
            <div>
              <p className="text-gray-600 text-sm">
                Page {currentPage} of {Math.ceil(guru.length / itemsPerPage)}
              </p>
            </div>
          </div>
        </div>
