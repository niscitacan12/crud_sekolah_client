...
function TabelSiswa() {
  ...
  const deleteSiswa = async (id) => {
    ...
    await Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://localhost:7000/api/data_siswa/hapus/${id}`, {
            headers: {
  ...
  const currentItems = siswa.filter((dataSiswa) =>
    dataSiswa.nama_siswa.toLowerCase().includes(searchTerm.toLowerCase())
  ).slice(indexOfFirstItem, indexOfLastItem);

  const paginate = (pageNumber) => {
    if (pageNumber < 1 || pageNumber > Math.ceil(siswa.length / itemsPerPage)) return;
        ...
        <div className="bg-white shadow-md rounded-lg p-4">
          <div className="bg-white shadow-md rounded-lg p-4 flex justify-between items-center">
              ...
              <div className="flex items-center -space-x-4 hover:space-x-1">
                <Link to={`/tabelSiswa/tambahSiswa`}>
              ...
              <tbody>
                {currentItems
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{dataSiswa.nama_siswa}</td>
                    ...
                    <td className="whitespace-nowrap text-center py-2">
                      <div className="flex items-center -space-x-4 hover:space-x-1">
                        <Link to={`/tabelSiswa/ubahSiswa/${dataSiswa.id}`}>
                          <button className="z-20 block rounded-full border-2 border-white bg-blue-100 p-4 text-blue-700 transition-all hover:scale-110 focus:outline-none focus:ring active:bg-blue-50"
                          >
                              <FaPenSquare className="z-20" title="Edit" />
                          </button>
                        </Link>
                        <Link to={`/tabelSiswa/detailSiswa/${dataSiswa.id}`}>
                          <button className="z-20 block rounded-full border-2 border-white bg-blue-100 p-4 text-blue-700 transition-all hover:scale-110 focus:outline-none focus:ring active:bg-blue-50"
                          >
                        ...
                        onClick={() => deleteSiswa(dataSiswa.id)}
                        >
          ...
          <div className="flex justify-between items-center mt-4">
            <div>
              ...
              <button
                onClick={() => paginate(currentPage + 1)}
                disabled={currentPage === Math.ceil(siswa.length / itemsPerPage)}
                className={`px-3 py-1 rounded-md bg-blue-500 text-white ${
                  currentPage === Math.ceil(siswa.length / itemsPerPage) ? "opacity-50 cursor-not-allowed" : ""
                }`}
              >
            ...
            <div>
              <p className="text-gray-600 text-sm">
                Page {currentPage} of {Math.ceil(siswa.length / itemsPerPage)}
              </p>
            </div>
          </div>
        </div>
